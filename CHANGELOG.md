...
- Workers now exit promptly on `SIGTERM` instead of having to be `SIGKILL`ed. Two issues conspired previously: `Sleeper.sleep()` used `time.sleep()` which (since PEP 475) retries on EINTR, so a worker sleeping until its next iteration ignored the signal until the full interval elapsed; and any in-progress `executor.execute()` could not be interrupted by the existing flag check. The signal handler now (a) sets a `threading.Event` that `Sleeper.sleep()` waits on instead of `time.sleep`, breaking the sleep immediately; and (b) raises `SIGALRM` so the `CheckExecutor` timeout handler raises `CheckTimeout` and returns control to the loop near-instantly. `CheckExecutor` was updated to restore `SIGALRM` to `SIG_IGN` after each call (and the worker installs `SIG_IGN` as its baseline at startup) so a stray `SIGALRM` between iterations is harmless.
- The worker's `_cleanup` handler is now idempotent. Previously a second termination signal arriving during shutdown (Ctrl-C on a foreground exacheck sends `SIGINT` to the process group *and* the master then sends `SIGTERM` via `worker.terminate()`) would invoke `_cleanup` twice. Each call raised `SIGALRM` → `CheckTimeout`, and the second `CheckTimeout` fired inside Loguru's `emit()` while it was writing the log entry produced by the first one, surfacing as a "Logging error in Loguru Handler" traceback. The handler now early-returns when `_termination_requested` is already set.
- Apprise integration now checks the return values of `apprise.add()` and `apprise.notify()`. Both can fail without raising — `add()` returns `False` for many invalid URL configurations, and `notify()` returns `False` if any configured target rejected delivery (e.g. webhook non-2xx). Previously a target that Apprise silently rejected during setup would disappear from the notification graph without a log entry, and a delivery failure that didn't raise was silently logged as "Notification sent". Both paths now emit an `error`-level log.
- Notifications are dispatched asynchronously. `Notifications.notify` now enqueues a notification onto an in-process queue and returns immediately; a background thread per process performs the actual Apprise call. A slow or hung notification backend (Slack webhook, etc.) no longer stalls the worker's health-check loop. Master and worker shutdown paths drain pending notifications with bounded timeouts (5 s and 3 s respectively) before exiting, and the master also drains the old Notifications object before replacing it on a notifications-config reload.
- Running workers no longer keep using their pre-fork copy of the `Notifications` object after the master reloads the notifications configuration. The master now pushes a `NotificationsUpdate` message down each worker's existing config queue when the `notifications:` section changes; the worker drains its old Apprise instance and rebuilds it with the new targets on the next loop iteration. Previously, a notifications-config reload only affected the master's notifications — worker check-state notifications continued to be sent to the stale URLs until the worker was restarted for an unrelated reason.
